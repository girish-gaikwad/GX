...
const DialogTitle = ({ children }) => {
  ...
  return (
    <div
      className={`text-xl font-semibold ${theme === 'dark' ? 'text-white' : 'text-black'} flex justify-between items-center`}
    >
      {children}
    </div>
  );
...
const DialogDescription = ({ children }) => {
  ...
  return (
    <div className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-400'} mt-2`}>{children}</div>
  );
};
...
const DialogFooter = ({ children }) => {
  ...
  return (
    <div
      className={`mt-6 flex justify-end gap-3 ${theme === 'dark' ? 'text-white' : 'text-black'}`}
    >
      {children}
    </div>
  );
  ...
  return (
    <div className="my-6">
      <div className="flex justify-between mb-2">
        ...
        <span className={`font-medium ${value < basePrice ? 'text-green-400' :
...
const Toast = ({ message, show, onClose }) => {
  ...
  return (
    <div className="fixed bottom-4 right-5 z-50 animate-slide-up">
      ...
      <div className="relative z-10 container mx-auto  py-12">
        ...
        <div className="text-center mb-12">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-4xl font-bold text-gray-900 dark:text-gray-100">Global Logistics Partner Finder</h1>
          </div>
        ...
        <div
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Quick Search</h2>
          </div>
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            {[
              ...
              <button
                ...
                className={
                  isDark
                    ? "px-4 py-2 bg-gray-700/50 text-gray-300 rounded-full hover:bg-indigo-500/20 hover:text-indigo-400 transition-colors"
                    : "px-4 py-2 bg-gray-200 text-gray-700 rounded-full hover:bg-indigo-500/20 hover:text-indigo-400 transition-colors"
                }>
                {text}
              </button>
            ))}
          ...
          <form id="search-form" onSubmit={handleSearch}>
            <div className="flex gap-3">
              <input
                ...
                className="flex-1 px-4 py-3  rounded-xl text-gray-900 dark:text-gray-100 placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                required
              />
              <button
                type="submit"
            ...
            {results.length > 0 ? (
              results.map((partner) => (
                <div
                  ...
                  className={
                    isDark
                      ? "bg-gray-800/40 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6 hover:border-indigo-500/50 transition-colors"
                      : "bg-white backdrop-blur-sm rounded-xl border border-gray-300/50 p-6 hover:border-indigo-500/50 transition-colors"
                  }
                >
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-1">{partner.partner_name}</h3>
                    ...
                    <div className="text-right">
                      ...
                      <div className="flex items-center gap-2 text-gray-500 dark:text-gray-300 mt-1">
                        <Clock size={16} />
                  ...
                  <div className="flex flex-wrap items-center gap-3 mb-4">
                    ...
                    <div className="text-gray-500 dark:text-gray-300">{partner.service_type}</div>
                  </div>
                  ...
                  <div className="flex justify-end">
                    <button
                      ...
                      className={
                        isDark
                          ? "flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-500 transition-colors"
                          : "flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-500 transition-colors"
                      }
                    >
        ...
        <Dialog open={isNegotiateOpen} onClose={() => setIsNegotiateOpen(false)}>
          ...
          <textarea
            ...
            className={`w-full mt-4 px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 ${
              isDark
                ? "bg-gray-700/50 text-gray-200 border-gray-600 placeholder:text-gray-400"
                : "bg-gray-100 text-gray-900 border-gray-200 placeholder:text-gray-500"
            }`}
            minLength={120}
          />
          ...
          <DialogFooter>
            <button
              onClick={() => setIsNegotiateOpen(false)}
              className="px-4 py-2 text-gray-400 dark:text-gray-500 hover:text-white dark:hover:text-gray-200 transition-colors"
            >
              Cancel
            </button>
            <button
              ...
              className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-500 transition-colors disabled:bg-gray-600 dark:disabled:bg-gray-700 disabled:cursor-not-allowed"
            >
              Send Message
            </button>
          </DialogFooter>
